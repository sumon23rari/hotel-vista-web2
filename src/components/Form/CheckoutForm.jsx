...
console.log(bookingInfo.roomId,'sdfsd')
  // Create Payment Intent
useEffect(()=>{
if (bookingInfo?.price>0) {
  console.log(bookingInfo?.price,'adfsdfs')
  ...
  const handleSubmit = async event => {
    ...
    if (paymentIntent?.status === 'succeeded') {
      ...
      try {
        ...
        // update room status in db
      const abc=  await updateStatus(bookingInfo?.roomId,true);
      console.log('abc',abc)
        const text=`booking success ${paymentIntent.id}`;
        toast.success(text);
